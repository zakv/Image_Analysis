function Main_PCO_Pixelfly_USB_flu_Synth_Ramp(run_config,image_instance_data)
...
n_shots_per_freq=2; %3; %Number of shots to take at each frequency
frequency_list=9405:5:9800; %Frequencies to set the synth to in order, in MHz

%Configure GPIB parameters
boardindex=0; %Which GPIB Computer Board to use (we only have 1)
primaryaddress=10; %GPIB address of synthesizer

%Close any existing connections to that GPIB device so we can open a new one
existing_connections = instrfind('Name',sprintf('GPIB%d-%d',boardindex,primaryaddress));
fclose(existing_connections);
%Open the new connection
synth_gpib = gpib('ni',boardindex,primaryaddress); %Create a nwe GPIB object instance to talk to the synth
fopen(synth_gpib); %Open the connection to the board
...
imacount=n_shots_per_freq*length(frequency_list); % imacount=run_config.imacount;
...
%here is the loop
for n=1:imacount
    %First, set the correct synth frequency
    freq_index=floor( (n-1)/n_shots_per_freq )+1;
    next_freq=frequency_list(freq_index); %Frequency for this upcoming shot
    fprintf(synth_gpib,':FREQuency:CW %f MHz',next_freq);
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(errorCode==0)
        ...
        if(errorCode)
            ...
            disp(['PCO_GetImageEx image ' int2str(n) ' done, freq was ' num2str(next_freq)]);
        end
    end
...
%         dlmwrite(pic.noise_image_filename, part3,'delimiter', '\t');
        shot_index=mod(n-1,n_shots_per_freq)+1;
        prefix=strcat(savingname, sprintf('synth%f_%d_',next_freq,shot_index));
