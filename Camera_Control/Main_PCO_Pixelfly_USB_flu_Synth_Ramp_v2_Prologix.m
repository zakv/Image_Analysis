...
sport.Timeout = 0.5; %Set timeout to this many seconds
fopen(sport);
fprintf(sport, '++mode 1'); %Set prologix to be in "controller" mode
fprintf(sport, sprintf('++addr %d',GPIB_address)); %Set controller to talk to correct GPIB address
fprintf(sport, '++auto 0'); %Turns off the auto-read function which messes things up I think
...
%     freq_index=floor( (n-1)/n_shots_per_freq )+1;
    freq_index=floor( (n-1-n_warmups)/n_shots_per_freq )+1;
    freq_index=max(freq_index,1); %avoid indexing errors during warmup shots
    next_freq=frequency_list(freq_index); %Frequency for this upcoming shot
    fprintf(sport,':FREQuency:CW %f MHz\n',next_freq);
