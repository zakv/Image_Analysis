...
{
  unsigned long dwPacketDelay;                 // delay between image stream packets (number of clockticks of a 100MHz clock;
                                       // default: 2000 -> 20us, range: 0 ... 8000 -> 0 ... 80us)
  unsigned long dwResendPercent;               // Number of lost packets of image in percent. If more packets got lost,
                                       // complete image will be resent or image transfer is failed (default 30).
  unsigned long dwFlags;                       // Bit 0:   Set to enable packet resend
  ...
  unsigned long dwDataFormat;                  // DataFormat: default:  0x01100007
  ...
  unsigned long dwCameraIPAddress;             // Current Ip Address of the Camera
									   //  (can not be changed with Set_Transfer_Param() routine )
  unsigned long dwUDPImgPcktSize;			   // Size of an UDP Image packet
									   //  (can not be changed with Set_Transfer_Param() routine )
  unsigned __int64 ui64MACAddress;               // Settings are attached to this interface
