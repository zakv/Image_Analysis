...
%         temppart=-1*log(abs(part1-part3)./abs(part2-part3));
        ...
        hProfile = sum(atompart,1); hProfile = reshape(hProfile,1,[]);
        vProfile = sum(atompart,2); vProfile = reshape(vProfile,1,[]);
        ...
        txtFitResults = { sprintf('w_H = %0.1f pix',hfit(2)) , 
                          sprintf('w_V = %0.1f pix',vfit(2)) , 
                          sprintf('T_H = %0.2f \mu{}K',hfit(2)^2/TOF^2*tempT) , 
                          sprintf('T_V = %0.2f \mu{}K',vfit(2)^2/TOF^2*tempT) 
                           };
...
%         title(title_string,'FontSize',30);
        ...
        % Urvoy Beginning of new plotting code with direct fitting 20180807
        figure(6)
        axBox = [0.1300 0.1100 0.7750 0.75]; imageWidth = .7;
        pos1 = [ axBox(1) axBox(2)+(1-imageWidth)*axBox(4) imageWidth*axBox(3) imageWidth*axBox(4) ];
        pos2 = [ axBox(1) axBox(2) imageWidth*axBox(3) (1-imageWidth)*axBox(4) ];
        pos3 = [ axBox(1)+imageWidth*axBox(3) axBox(2)+(1-imageWidth)*axBox(4) (1-imageWidth)*axBox(3) imageWidth*axBox(4) ];
        pos4 = [ axBox(1)+imageWidth*axBox(3) axBox(2) (1-imageWidth)*axBox(3) (1-imageWidth)*axBox(4) ];
...
%         axes('Position',pos4)
        ...
        % Urvoy End of new plotting code with direct fitting 20180807
    end    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%For another new picture in the same for sequence.%%%%%%%%
    if (twoimage==0)
